...
```bash
git clone git@github.com:AdithVasu/ScienceOlympiadManagementSystem.git
cd ScienceOlympiadManagementSystem
...
```env
PORT=6060
DATABASE_URI=mongodb://127.0.0.1:27017/science-olympiad
ACCESS_TOKEN_SECRET=replace-with-a-long-random-secret
REFRESH_TOKEN_SECRET=replace-with-a-different-long-random-secret
FRONTEND_URL=http://localhost:5173
EMAIL_ACCOUNT=your-email-account
EMAIL_APP_PASS=your-email-app-password
```
...
| Variable | Purpose |
| --- | --- |
| `PORT` | API listening port; defaults to `6060` |
| `DATABASE_URI` | MongoDB connection string |
| `ACCESS_TOKEN_SECRET` | Signs short-lived access tokens |
| `REFRESH_TOKEN_SECRET` | Signs refresh tokens |
| `FRONTEND_URL` | Allowed frontend origin for CORS |
| `EMAIL_ACCOUNT` | Account used for password-reset email |
| `EMAIL_APP_PASS` | Email provider app password |
...
The API is available at `http://localhost:6060`.

### Frontend environment

The frontend uses `http://localhost:6060` when no API URL is provided. To override it, create `frontend/.env.local`:
